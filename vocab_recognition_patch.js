...
function ensureButtons(){['vrecKnown','vrecFuzzy','vrecUndo','vrecShow','vrecDue'].forEach(id=>{const b=document.getElementById(id);if(b&&b.disabled){b.disabled=false;b.removeAttribute('disabled')}})}
let lastSig='';
function render(){const box=document.getElementById('vrecBox');if(!box)return;ensureButtons();const w=findWord(),d=load(),s=stats(),x=w?d[keyOf(w)]:null;
 ...
 const st=document.getElementById('vrecStatus');if(st)st.innerHTML='<span class="vrec-pill">认识 '+s.known+'</span><span class="vrec-pill">模糊 '+s.fuzzy+'</span><span class="vrec-pill">到期 '+s.due+'</span>'+(x?'<span class="vrec-pill">当前：'+(x.state==='known'?'认识':'模糊')+'</span>':'');
