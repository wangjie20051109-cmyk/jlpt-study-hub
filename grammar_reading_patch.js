...
'～と思います':'～とおもいます','～とは限らない':'～とはかぎらない','～一方で':'～いっぽうで','～に応じて':'～におうじて','～以上':'～いじょう','～に伴って':'～にともなって','～に対して':'～にたいして','～に違いない':'～にちがいない','～ざるを得ない':'～ざるをえない','～に至るまで':'～にいたるまで','～を余儀なくされる':'～をよぎなくされる'
};
...
function grammarPatternKana(p=''){
 ...
 return /[一-龯々]/.test(out)?'读音待补充':out;
}
`;
...
const replacement="$('gl').textContent=x[0];$('gsource').textContent=x[8]==='supp'?'扩展语法库':(remote?'完整语法库':'核心精讲');$('gpat').textContent=x[1];let gpk=grammarPatternKana(x[1]);$('gpatk').textContent='平假名：'+gpk;$('gzh').textContent=((remote||x[8]==='supp')?'英文原义：':'中文：')+x[2];";
